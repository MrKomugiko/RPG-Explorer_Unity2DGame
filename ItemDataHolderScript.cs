...
public class ItemDataHolderScript : MonoBehaviour
{
    ...
    public void UseItem() {
        ...
        if (itemData.item.type.ToString() == "Default") {
            ...
            Amount -= 1;
            PlayerInstance.inventory.Container[ItemSlotIndex].amount = Amount;
            PlayerInstance.inventory.Container[ItemSlotIndex].item.name = Name;
          ...
          //  Debug.Log("all is used, empty slot gonna disapear now");
            PlayerInstance.inventory.RemoveEmptyContainer(ItemSlotIndex);
            Destroy(this.gameObject);
        }
    }
    private void TryMakeItemAbleToUse() {
        if (itemData.item.type.ToString() == "Equipment") {
            _itemMeedAllRequirments = GameObject.Find("PlayerInventory").GetComponent<InventoryScript>().CheckItemRequirmentsIfIsAbleToWear((EquipmentObject)itemData.item);
            if (_itemMeedAllRequirments == false) {
    ...
    private void UsingFoodItem() {
       ...
       // Debug.Log($"You eat <{foodItem.name}>.");
        ...
        PlayerInstance.GetHealed(foodItem.restoreHealthValue);
    ...
    private void UsingEquipmentItem() {
        ...
        if (PlayerInstance.CheckIfItemWithThisGenreAlreadyIsWeared(itemData.item)) {
            if (wearableITem.equipmentGenre.ToString() == "Ring") {
                Debug.Log("You want wear another ring");
                if(PlayerInstance.listOfEquipmentInUse.Where(p=>p.equipmentGenre.ToString() == "Ring").Count() == 3) {
                    Debug.Log("You already have 3 rings, now one of them swap with new ring");
                    PlayerInstance.SwapEquipmentPositionFromEqToInventory(itemData.item);
                }
            } else {
                PlayerInstance.SwapEquipmentPositionFromEqToInventory(itemData.item);
            }
        }
        PlayerInstance.WearEquipment(itemData.item);
    }
   ...
   [SerializeField] private GameObject ItemActionsListWindow;
    ...
    public void DisplayItemActionListWindow(ItemObject item) {
        ItemActionsListWindow = gameObject.GetComponentInParent<InventoryScript>().itemActionListWindow;
        ItemActionsListWindow.GetComponent<ItemActionsWindowScript>().OpenAndPassItemDataToActionListWindow(item,ItemSlotIndex);
