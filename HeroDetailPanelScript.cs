...
public class HeroDetailPanelScript : MonoBehaviour
{
    ...
    private void PutCurentlyUsingEquipmentIntoSlot() {
        ...
        foreach (var equipmentItem in playerInstance.listOfEquipmentInUse) {
            ...
            if(slotName == "Ring") {
                if(playerInstance.listOfEquipmentInUse.Where(p=>p.equipmentGenre.ToString() == "Ring").Count() == 1) {
                    slotName = RingsSlotsName.Ring1Slot.ToString();
                }
                if (playerInstance.listOfEquipmentInUse.Where(p => p.equipmentGenre.ToString() == "Ring").Count() == 2) {
                    slotName = RingsSlotsName.Ring2Slot.ToString();
                }
                if (playerInstance.listOfEquipmentInUse.Where(p => p.equipmentGenre.ToString() == "Ring").Count() == 3) {
                    slotName = RingsSlotsName.Ring3Slot.ToString();
                }
            }
            GameObject slot = this.transform.GetChild(3).Find(slotName).gameObject;
